...
public:
  ...
  size_t output_matches(string const& pattern, OutIter&& out) const;
  ...
  trie(trie&& n) : children (move (n.children))
  {
  }
  ...
  void clear(){
    ...
    {
      trie* node_ptr = bfs.front();
      for (auto i: node_ptr->children){ // goes through all child nodes
        if (i.second != nullptr) // not empty
            bfs.push (i.second); //pushes child node to queue
